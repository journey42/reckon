...
from rhiz.state.base import (
    User,
)  # Import the module where your SQLAlchemy models are defined
...
def send_password_reset_email(session: rx.session, user: User, reset_page_url: str):
    ...
    try:
        ...
        send_result = (
            poller.result()
        )  # You might need to handle this asynchronously depending on your application
...
def send_welcome_email(
    session: rx.session, username: str, email: str, beta_site_url: str
):
    # Create an EmailClient
    email_client = EmailClient.from_connection_string(CONNECTION_STRING)
    ...
    message = {
        "content": {
            "subject": "Welcome to the Rhiz Beta!",
            "plainText": f"Hello {username},\n\nYou have been granted access to the Rhiz beta! You can now log in using your existing credentials.\n\nAccess the beta site here: {beta_site_url}\n\nWe look forward to your feedback!",
            "html": f"<p>Hello <strong>{username}</strong>,</p><p>You have been granted access to the Rhiz beta! You can now log in using your existing credentials.</p><p>Access the beta site <a href='{beta_site_url}'>here</a>.</p><p>We look forward to your feedback!</p>",
        },
    ...
    try:
        ...
        send_result = (
            poller.result()
        )  # Handle this result according to your application's needs
        if send_result["status"] == "Succeeded":
            return True
        else:
            return False
